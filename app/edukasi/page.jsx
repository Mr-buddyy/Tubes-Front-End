...
function page() {
    ...
    useEffect(() => {
        // Setelah data diupdate, filter berdasarkan kata kunci
        const filtered = data.filter((item) => item.judul.toLowerCase().includes(searchKeyword.toLowerCase()));
                ...
                {filteredData.length < 1 ? (
                    ...
                    filteredData.map((item, index) => (
                        <div key={index} className=" flex flex-col gap-5 bg-white p-10 rounded-tl-3xl shadow-[0_10px_60px_-5px_rgba(0,92,230,0.76)">
                            ...
                            <div className="">
                                <Link href={`/detail/${item.id}`} className="bg-black py-2 px-5 rounded-md text-white">
                                    Detail
                                </Link>
                            </div>
                        </div>
